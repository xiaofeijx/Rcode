...
需要knitr,tidyverse,xtable等几个包,需要提前安装
...
#病种与诊断分类
#只能实验室诊断
shiyanshi <- df$疾病名称 %in% c("Ⅰ期梅毒","Ⅱ期梅毒","III期梅毒","胎传梅毒","隐性梅毒","淋    病","艾滋病","HIV","肺结核病原学阳性","脊灰") & df$病例分类 != "确诊病例"
...
#显示除麻疹、未痰检和其它疾病的疑似病例
yishi <- !(df$疾病名称 %in% c("肺结核病原学阴性","肺结核无病原学结果","麻疹","风疹","其它疾病:")) & (df$病例分类 =="疑似病例")
...
df4 <- df %>% filter(select==TRUE) %>% select(患者姓名,疾病名称,报告单位,出生日期,zj) %>%   mutate(sfzsr = ymd(str_sub(zj,7,14)),出生日期=ymd(出生日期)) %>% select(患者姓名,疾病名称,报告单位,出生日期,sfzsr) %>% filter(sfzsr!=出生日期) 
print(df4,width = Inf)
