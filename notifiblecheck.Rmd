...
select(totaldf,showcol_1) %>% filter(difftime(totaldf$报告卡录入时间,totaldf$诊断时间,units="hours") >= 2 & classone)
...
```{r}
#显示报告时间大于24小时的疾病
totaldf[difftime(totaldf$报告卡录入时间,totaldf$诊断时间,units="days") >= 1, showcol_1]
```
...
modifieddf <- totaldf[!is.na(totaldf$订正报告时间),]
mclassone <- modifieddf$疾病名称 %in% c("鼠 疫","霍 乱","传染性非典","脊    灰","肺炭疽")
modifieddf[difftime(modifieddf$订正报告时间,modifieddf$诊断时间,units="hours") >= 2 & mclassone, showcol_1]

```
**订正报告卡时限检查（乙丙类）**
```{r}
#显示订正报告的传报卡，报告时间超过24小时
modifieddf[difftime(modifieddf$订正报告时间,modifieddf$诊断时间,units="days") >= 1, showcol_1]
```
