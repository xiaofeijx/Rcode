...
select(df,showcol) %>% filter(rarefilter) 
```
...
select(totaldf,showcol) %>% filter(difftime(totaldf$报告卡录入时间,totaldf$诊断时间,units="hours") >= 2 & classone)
...
```{r}
#显示报告时间大于24小时的疾病
totaldf[difftime(totaldf[,30],totaldf[,19],units="days") >= 1, showcol]
```
...
modifieddf[difftime(modifieddf[,38],modifieddf[,37],units="hours") >= 2 & mclassone, showcol]

```
**订正报告卡时限检查（乙丙类）**
```{r}
#显示订正报告的传报卡，报告时间超过24小时
modifieddf[difftime(modifieddf[,30],modifieddf[,19],units="days") >= 1, showcol]
```
...
select(df,showcol) %>% filter(diseasefilter)
```

**乙肝、血吸虫病，丙肝急慢性不能为未分类**
```{r}
jimanfilter <- (df$疾病名称 %in% c("乙肝","血吸虫病","丙肝")) & (df$病例分类2 =="未分类")
select(df,showcol) %>% filter(jimanfilter)
```
...
select(df,showcol) %>% filter(parentfilter)
```
...
select(df,showcol) %>% filter(beizhufilter)
```
...
select(df,showcol) %>% filter(phonenumber)
```
