...
df <- read.csv("Report2.csv",header=T,stringsAsFactors=F,na.strings = ".")
...
occufilter <- (df$职业 %in% c("幼托儿童","学生","干部职员","工人","民工","教师","医务人员")) & (nchar(df$患者工作单位)<=2 | (df$患者工作单位=="不详") |(df$患者工作单位=="无") |(df$患者工作单位=="不清楚"))
...
beizhufilter1 <- df$疾病名称 %in% c("流行性感冒","其它感染性腹泻病","肝炎（未分型）") & (df$病例分类 =="实验室确诊病例") & nchar(df$备注) < 2
