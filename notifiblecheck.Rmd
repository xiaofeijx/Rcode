...
df$area <- factor(df$area,labels=c("南湖区","秀洲区","嘉善县","海盐县","海宁市","平湖市","桐乡市"))
...
**14岁以下填电话号码和家长姓名**
```{r}
agephonefilter <- df$age <=14 & (((nchar(df$联系电话)!= 11 & nchar(df$联系电话)!= 8) | is.na(df$联系电话)) | nchar(df$患儿家长姓名)<2 )
select(df,c(4,5,11,15,21,28)) %>% filter(occufilter3)

#填了家长姓名却大于14岁
parentfilter <- df$age >14 & nchar(df$患儿家长姓名)>= 2
...
beizhufilter1 <- df$疾病名称 %in% c("流行性感冒","其它感染性腹泻病","肝炎（未分型）","细菌性痢疾","伤 寒","副伤寒") & (df$病例分类 =="实验室确诊病例") & nchar(df$备注) < 2
...
```{r}
#只检查不是性病、其它感染性腹泻的报卡
phonefilter <- df$疾病名称 %in% c("淋    病","Ⅰ期梅毒","Ⅱ期梅毒","III期梅毒","胎传梅毒","隐性梅毒","尖锐湿疣","生殖器疱疹","生殖道沙眼衣原体感染","其它疾病:","其它感染性腹泻病")
df <- df[!phonefilter,]
