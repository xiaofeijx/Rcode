...
```
本次检测的报卡时间范围为`r max(luru[1],dingzheng[1])`至`r luru[2]`
...
**显示2岁以下不是散居和5岁以上为散居**
```{r}
agefilter2 <- (!(df$职业 %in% c("散居儿童")) & df$age < 2) | ((df$职业 %in% c("散居儿童")) & df$age>=5) 
...
**年龄与病种的关系**  
根据2014年的报卡，大致关系如下：新生儿破伤风小于28天，胎传梅毒小于5岁，手足口病小于30岁，AFP小于15岁，其它梅毒、艾滋病、丙肝、肺结核、肝炎大于15岁。  
```{r}
...
select(df,showcol_2) %>% filter(poshangfeng|tcmeidu|szkb|otherfilter)
```
...
```{r}
danweifilter <- grep("小学|公司",df$患者工作单位)
#先选出有小学或公司两字的记录
youtuodf <- df[danweifilter,]
select(youtuodf[youtuodf$职业 %in% c("幼托儿童","教师"),],showcol_2)
```
