...
#填了单位，但职业不对的
occufilter2 <- !(df$职业 %in% c("幼托儿童","学生","干部职员","工人","民工","教师","医务人员")) & (nchar(df$患者工作单位) >=2 &  !(df$患者工作单位=="无"))
select(df,c(4,7,9,10,15,19,21,28,30)) %>% filter(occufilter2)
```
...
```{r}
#病种与诊断分类
shiyanshi <- ((df$疾病名称 %in% c("乙肝","Ⅰ期梅毒","Ⅱ期梅毒","III期梅毒","胎传梅毒","隐性梅毒","淋    病","艾滋病","ＨＩＶ","涂（+）","仅培阳","脊    灰")) & !(df$病例分类 %in% c("实验室确诊病例")))
lingchuang <- (df$疾病名称 %in% c("AFP","菌（-）")) & (df$病例分类 !="临床诊断病例")
yishi <- (df$疾病名称 == "未痰检") & (df$病例分类 !="疑似病例")

diseasefilter <- shiyanshi | lingchuang | yishi
...
beizhufilter1 <- df$疾病名称 %in% c("流行性感冒","其它感染性腹泻病") & (df$病例分类 =="实验室确诊病例")
beizhufilter2 <- df$疾病名称 %in% c("间日疟","恶性疟","疟疾（未分型）","登革热")
beizhufilter <- beizhufilter1 | beizhufilter2
select(df,c(4,7,9,16,21,46)) %>% filter(beizhufilter)
```

**电话号码检查**
```{r}
#显示电话号码不是11位或8位的报卡,但不现实电话号码为空的报卡
